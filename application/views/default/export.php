...
if(confirm_url_id(1)) {
    ...
    if(in_array($initials, ["INV", "POS"])) {
		...
		$sales = $pos->prepare("
			...
			WHERE order_id = ? {$branchAccess} {$accessLimit}
		");
	} else {
		...
		$sales = $pos->prepare("
			...
			WHERE request_id = ? {$branchAccess} {$accessLimit}
		");
	}
	...
	<tbody>
		...
		<tr>
			<td align=\"left\" width=\"35%\">
				<span><strong>ID:</strong> #{$sales_id}</span><br><br><span><strong>Date:</span> ".date("jS M Y h:iA", strtotime($result->request_date))."</span><br>
			</td>
...
<table border=\"0\" width=\"100%\" cellpadding=\"20px\" cellspacing=\"0px\">
	<tbody>
		<tr>
			<td style=\"background-color:#f4f4f4\" valign=\"top\">&nbsp;List of items for this 
